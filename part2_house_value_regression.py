    ...
    def fit(self, x_train, y_train, x_val=None, y_val=None):
        ...
        for epoch in range(self.nb_epoch):
            ...
            if has_val:
                ...
                print("train/val loss", epoch, train_loss, val_loss)
                plt_y_train.append(np.sqrt(train_loss.item()) * self.y_norm)
                plt_y_val.append(np.sqrt(val_loss.item()) * self.y_norm)
        ...
        plt.plot(plt_x, plt_y_val, label='Validation')
        plt.xlabel("Epoch")
        plt.ylabel("RMSE")
        plt.grid()
        plt.legend(title='Error')
        # plt.savefig('Plots/Plot 1.png', dpi=300)
        plt.show()
